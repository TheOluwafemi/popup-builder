...
<template>
    ...
    >
        ...
        <template v-if="info.titleFields?.length">
            <div v-for="(item, index) in info.titleFields" :key="index" class="title" :style="{ transform: item.transform }">
                <h3>{{ item.value }}</h3>
            </div>
        </template>
        ...
        <template v-if="info.subtitleFields?.length">
            <small v-for="(item, index) in info.subtitleFields" :key="index" class="subtitle" :style="{ transform: item.transform }">{{
                item.value
            }}</small>
        </template>
    </section>
